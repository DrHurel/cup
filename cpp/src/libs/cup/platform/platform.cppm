...
// Re-exported, not merely imported: cup::error::Error appears in this module's
// exported signatures (std::expected<RawMode, Error>), so it is part of the
// interface. A plain `import` compiles the module itself but leaves consumers
// unable to resolve the type — under GCC 14 that surfaces as a "Bad file data"
// failure reading this module's BMI rather than as a diagnostic here.
