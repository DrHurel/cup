...
std::string module_std_setup(const project::Config& cfg) {
    if (cfg.uses_std_module()) {
        return "# `import std;` requires CMake >= 3.30 (CMAKE_CXX_MODULE_STD support for GCC)\n"
               ...
               "\n"
               "# Opt in to CMake's still-experimental `import std` support. The gate is a\n"
               "# CMake-version-specific UUID (CMAKE_EXPERIMENTAL_CXX_IMPORT_STD) that CMake\n"
               "# rotates almost every release and exposes no way to query — `cup configure`/\n"
               "# `cup build` detects the installed CMake and supplies it via -D before this\n"
               "# file's project() call runs, rather than baking a value here that would go\n"
               "# stale the moment it differs from the CMake this project was scaffolded with.\n"
               "\n"
