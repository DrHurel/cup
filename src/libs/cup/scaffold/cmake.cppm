...
// Cmake.cpp) import cup.ui and use <regex>/<fstream> internally. Unlike the
// earlier design, <filesystem> here turned out to be safe alongside
// render.cppm's and compiler.cppm's own <filesystem> — the cup.project
// :config/:io BMI-merge failure (constraint 1) does not reproduce for three
// interface partitions all reaching <filesystem> in this module; verified by
// building on GCC 14 before this changed from plain std::string paths.
...
[[nodiscard]] std::optional<std::vector<std::string>> read_file_lines(const std::filesystem::path& path);
...
[[nodiscard]] std::expected<void, error::Error> ensure_line_before(const std::filesystem::path& root,
                                                                   const std::filesystem::path& path,
                                                                   std::string_view line,
...
[[nodiscard]] std::expected<bool, error::Error> remove_matching_line(const std::filesystem::path& root,
                                                                      const std::filesystem::path& path,
...
[[nodiscard]] std::expected<void, error::Error> add_module_source(const std::filesystem::path& root,
                                                                   const std::filesystem::path& path,
...
[[nodiscard]] std::expected<void, error::Error> add_header_source(const std::filesystem::path& root,
                                                                   const std::filesystem::path& path,
                                                                   std::string_view filename);
...
[[nodiscard]] std::expected<void, error::Error> ensure_header_lib_static(const std::filesystem::path& root,
                                                                         const std::filesystem::path& path,
...
[[nodiscard]] std::expected<void, error::Error> add_partition_import(const std::filesystem::path& root,
